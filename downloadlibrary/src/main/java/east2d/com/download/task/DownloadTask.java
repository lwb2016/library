...
import east2d.com.download.exception.DownloadException;
...
public class DownloadTask implements Runnable,DownloadListener,TaskDataOption {
    private String url;
    private String savePath;
    private TaskDataOption mTaskDataOption;
    ...
    public void setTaskDataOption(TaskDataOption taskDataOption) {
        mTaskDataOption = taskDataOption;
    }

    @Override
    public void run() {
        if(TextUtils.isEmpty(url))
            throw new DownloadException("Empty download url");
        if(TextUtils.isEmpty(savePath))
            throw new DownloadException("Empty download save path");
...
//            }
    ...
    public String generalFileName(String url) {
        if(mTaskDataOption!=null)
            return mTaskDataOption.generalFileName(url);
    ...
    public long getMax(String url, String savePath) {
        if(mTaskDataOption!=null)
            return mTaskDataOption.getMax(url,savePath);
        return 0;
    }
    ...
    public long getAvailable(String url, String savePath) {
        if(mTaskDataOption!=null)
            return mTaskDataOption.getAvailable(url,savePath);
        return 0;
    }
